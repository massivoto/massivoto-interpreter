...
| Scope | ✅ Complete | 100% |
| Requirements: Precedence Model | ✅ Complete | 3/3 |
| Requirements: Remove Mutual Exclusivity | ✅ Complete | 3/3 |
| Requirements: Interpreter - Filter Pattern | ✅ Complete | 4/4 |
| Requirements: New Reserved Args (retry, collect) | ✅ Complete | 6/6 |
| Acceptance Criteria | ✅ Complete | 14/14 |
| Theme | ✅ Defined | - |
...
- [x] AC-FP-01: Given `situations = ['rain overtake', 'monaco tunnel', 'pit stop', 'start grid']`
...
- [x] AC-FP-02: Given `@block/begin forEach=drivers->driver if={driver.points > 50}`,
      when parsed, then `BlockNode` has both `forEach` and `condition` populated (no error)

- [x] AC-FP-03: Given `@block/begin forEach=drivers->driver if={driver.points > 50}`,
...
- [x] AC-FP-11: Given `@ai/describe prompt="F1" output=result collect=results`,
      when parsed, then parser rejects with "Cannot use both output= and collect="

**Precedence independence from position:**

- [x] AC-FP-12: Given `@cmd/name retry=3 if={x} collect=r forEach=items->item`,
      when parsed, then all four reserved args are correctly extracted regardless of order

**Parsing:**

- [x] AC-FP-13: Given `@cmd/name retry=3`, when parsed, then `InstructionNode.retry` is a literal number expression with value 3

- [x] AC-FP-14: Given `@cmd/name collect=myResults`, when parsed, then `InstructionNode.collect` is an identifier node with value "myResults"

**General:**
- [x] All automated tests pass
- [x] Edge cases covered in `filter-pattern.edge.spec.ts`
